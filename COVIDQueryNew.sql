...
SELECT Location, SUM(newcases) AS totalcases, SUM(newdeaths) AS totaldeaths FROM [Tobiloba].[dbo].[COVID]
GROUP BY Location
...
-- JOIN the COVIDTotal and World population tables
SELECT a.Location, a.newcases, a.newdeaths, b.population AS Population
FROM [Tobiloba].[dbo].[COVIDTotal] a
FULL OUTER JOIN [Tobiloba].[dbo].[worldpop] b
ON a.Location = b.Location


-- Find the sum of newcases, newdeaths, population in the world
SELECT SUM(a.newcases) AS newcases, SUM(a.newdeaths) AS newdeaths, SUM(b.population) AS TotalPopulation
FROM[Tobiloba].[dbo].[COVIDTotal] a
FULL OUTER JOIN [Tobiloba].[dbo].[worldpop] b
ON a.Location = b.Location
